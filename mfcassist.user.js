...
//TODO:
//1. Need a better way for creating DOM elements
//2. Use the actual MFC settings instead of using a parallel set
//3. LocalStorage manager
//4. Figure out how the chatbox is actually getting populated in LoadPlayer
//5. Merge the sound playing code into the MAssist object (use g_tRef to get appropriate info)
//6. Merge chatbox scroll fix into this script
...
if (0 === window.location.href.indexOf('http://www.myfreecams.com/mfc2/static/player.html')) {
    //polyfill for unsafeWindow
     ...
     var maStyles = [
                    ...
                    '            background-image: -moz-linear-gradient(top, #ff8c00, #ffa500);',
    ...
    var MAssist = (function () {
        ...
        function init() {
            //the setTimeout here is to prevent a typeerror that occurs
            //if we wrap the function prior to the first load on the page
            setTimeout(function () {
        ...
        function setupHTML() {
            ...
            var body = document.querySelector('#videochat_container tbody');
            var fragment = create('<tr><td colspan=4><div id="ma-menu-container"><div id="ma-menu"><span class="brand">MFC Assistant</span><span class="model">Model: <span id="currentModel"></span></span><span class="stat">Top: <span id="highestTip" /></span><span class="stat">Contrib: <span id="topContributor" /></span></div><div class="menubutton"><div class="dropdown"><span>Countdown  <span id="menuCount"/></span><span class="caret"></span></div><div class="dropdown-menu"><ul><li><a id="setCountdown">Set Countdown</a></li><li><a id="postCurrentCount">Post Current Count</a></li><li><a id="autoPost">Enable AutoPost</a></li></ul></div></div></div></td></tr>');
            body.insertBefore(fragment, body.querySelector('tr'));
            body.appendChild(divAlert);
            spanHighestTip = document.querySelector('#highestTip');
            spanCurrentModel = document.querySelector('#currentModel');
            spanMenuCount = document.querySelector('#menuCount');
            spanTopContrib = document.querySelector('#topContributor');
            aAutoPost = document.querySelector('#autoPost');
            aPostCurrentCount = document.querySelector('#postCurrentCount');

            [ ].slice.call(document.querySelectorAll('.dropdown')).forEach(function (e) {
                e.addEventListener('click', function (event) {
                    clearMenus();
                    var menu = e.parentNode.querySelector('.dropdown-menu');
            ...
            function clearMenus() {
                [ ].slice.call(document.querySelectorAll('.dropdown-menu[data-open="true"]')).forEach(function (e) {
...
if (0 === window.location.href.indexOf('http://www.myfreecams.com/mfc2/static/top.html')) {
    ...
    setInterval(function () {
        ...
        if (!currentFriends.compareArrays(onlineFriends)) {
            //compile new people
            var offlineMsg;
            var offline = onlineFriends.diff(currentFriends).map(function (item) { return g_hUsers[item]['username']; });
            if (offline.length > 0)
                offlineMsg = offline.join(", ") + " went offline.";

            //compile people that left
            var onlineMsg;
            var online = currentFriends.diff(onlineFriends).map(function (item) { return g_hUsers[item]['username']; });
            if (online.length > 0)
